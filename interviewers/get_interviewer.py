...
def get_interviewers():
    ...
    if(interviewer_id == None):
        interviewers = dbstatements.run_select_statement("SELECT ja.company, i2.id, i2.interview_id, i2.job_app_id, i2.name, i2.job_position, i2.email, i2.phone_number, i2.other_contact_info, i2.notes FROM job_application ja INNER JOIN interviewer i2 ON i2.job_app_id = ja.id WHERE i2.user_id = ?", [user_id,])
    # If the user does send an interviewer id, get the interviewer that is owned by the user id and has that interviewer id
    else:
        interviewers = dbstatements.run_select_statement("SELECT ja.company, i2.id, i2.interview_id, i2.job_app_id, i2.name, i2.job_position, i2.email, i2.phone_number, i2.other_contact_info, i2.notes FROM job_application ja INNER JOIN interviewer i2 ON i2.job_app_id = ja.id WHERE i2.user_id = ? AND i2.id = ?", [user_id, interviewer_id])
