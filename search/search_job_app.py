...
def search_job_app():
    ...
    try:
        ...
        if(user_id == '' or search_input == ''):
            return Response("Please enter a valid search input.", mimetype="text/plain", status=400)
    except KeyError:
        traceback.print_exc()
        return Response("Incorrect or missing key.", mimetype="text/plain", status=403)
    except:
        traceback.print_exc()
    ...
    # If the user does send data, try to find all the user's job applications that matches or closely matches with their search input
        ...
        results = dbstatements.run_select_statement("SELECT id, company, job_posting_url, job_position, job_location, employment_type, salary_type, salary_amount, start_date, due_date, status, applied_date, notes, user_id FROM job_application WHERE user_id = ? AND company LIKE CONCAT('%', ?, '%') ORDER BY created_at DESC", [user_id, search_input])
